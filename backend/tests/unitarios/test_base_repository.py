...
class TestBaseRepository:
    ...
    def test_execute_insert_success(self, repository, mock_db_connection, mock_cursor):
        ...
        # Act
        result = repository.execute_insert(
            "INSERT INTO tabela (nome) VALUES (%s) RETURNING id",
            ('Novo Registro',)
        )
    ...
    def test_execute_update_success(self, repository, mock_db_connection, mock_cursor):
        ...
        # Act
        affected = repository.execute_update(
            "UPDATE tabela SET ativo = %s WHERE status = %s",
            (True, 'pendente')
        )
